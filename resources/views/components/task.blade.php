<div data-task-title="{{ $data['title'] }}" data-task-category="{{ $data['category']->title }}"
    ...
    data-task-date="{{$data['due_date']}}"
    data-bs-toggle="modal" data-bs-target="#taskModal"
    class="task {{ $data['is_done'] ? 'task_done' : 'task_pending' }}">
    <div class="title">
        <input type="checkbox" onchange="taskUpdate(this)" data-id="{{ $data['id'] }}" @if ($data && $data['is_done']) checked @endif />
    ...
    <div class="actions justify-content-end d-flex flex-row align-items-center" style="flex: 1;">
        ...
        {{-- botao excluir --}}
        <a data-bs-toggle="modal" data-bs-target="#deleteModal" style="cursor: pointer;" data-taskId="{{ $data['id'] }}" data-title="{{$data['title']}}" class="delete-task">
